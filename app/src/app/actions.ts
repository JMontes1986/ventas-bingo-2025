...
export async function getCashierWarning(activeUsers: ActiveDaviplataUsers): Promise<CashierWarningOutput | null> {
    noStore();
    try {
        ...
        if (dashboardDataRes.error || pendingOrdersRes.error) {
            console.error(`Failed to fetch data for warning: ${dashboardDataRes.error || pendingOrdersRes.error}`);
            // Return null or a default safe value instead of throwing, so the UI doesn't break
            return null;
        }
        
        const input: CashierWarningInput = {
            pendingDaviplataOrdersCount: pendingOrdersRes.data?.length ?? 0,
            avgDaviplataVerificationTime: dashboardDataRes.data?.avg_daviplata_verification_time ?? null,
            activeDaviplataUsersInCompletedState: activeUsers.states.completado,
            cashierEfficiency: dashboardDataRes.data?.avg_sale_time_by_cajero ?? []
        };
