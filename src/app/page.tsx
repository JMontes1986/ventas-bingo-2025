...
import { getArticles, getSalesByArticle, getAllReturns, countPendingDaviplataConversations, findDaviplataOrder, completeDaviplataOrder, recordSaleTransactional, getCashierWarning, countCompletedDaviplataOrders } from '@/app/actions';
import type { Article, CartItem, SaleData, Cajero, VentaConDetalles, ReturnDetail, DaviplataOrder, DaviplataOrderDetail, ArticleSale, ActiveDaviplataUsers, CashierWarningOutput } from '@/types';
...
export default function SalesPage() {
  ...
  const refreshAllData = useCallback(async (isManualRefresh = false) => {
    ...
    try {
        const [articlesResult, salesByArticleResult, returnsResult, convResult] = await Promise.all([
            ...
            getSalesByArticle(),
            getAllReturns(),
            user.can_access_ai_analysis ? countPendingDaviplataConversations(user) : Promise.resolve({count: 0})
        ]);

        if (articlesResult.error) throw new Error(`Artículos: ${articlesResult.error}`);
        if (salesByArticleResult.error) throw new Error(`Ventas: ${salesByArticleResult.error}`);
        if (returnsResult.error) throw new Error(`Devoluciones: ${returnsResult.error}`);
        if (convResult.error) throw new Error(`Conversaciones: ${convResult.error}`);


        setArticles(articlesResult.data || []);
        setArticleSales(salesByArticleResult.data || []);
        setReturns(returnsResult.data || []);
        setPendingConversations(convResult.count);
  ...
  useEffect(() => {
    if (user) {
        setIsLoading(true);
        refreshAllData(false).finally(() => setIsLoading(false));
    }
  }, [user, refreshAllData]);
  ...
  useEffect(() => {
    ...
    const presenceInterval = setInterval(async () => {
        try {
            ...
            const data: ActiveDaviplataUsers = await response.json();
            setActiveDaviplataUsers(data);
            ...
            if(user.can_access_dashboard || user.username.toLowerCase() === 'caja1' || user.username.toLowerCase() === 'caja2' || user.username.toLowerCase() === 'administrador') {
                 const warning = await getCashierWarning(data);
                 setCashierWarning(warning);
            }

        } catch (e) {
            console.error("Failed to fetch presence or completed count:", e);
        }
                ...
                {user.username.toLowerCase() !== 'entrada' && (
                  <TabsContent value="promotions" className="pt-4">
                    <PromotionsTab
                      ...
                      onAddToCart={addToCart}
                      getItemsInCart={getItemsInCart as (articleId: string) => number}
                     />
                  </TabsContent>
                )}
              </Tabs>
